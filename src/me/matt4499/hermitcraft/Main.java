...
public class Main extends JavaPlugin {
	...
	public void onEnable() { //Called when the plugin is enabled/loaded
		...
		Bukkit.getServer().getPluginManager().registerEvents(new ShulkerDeathListener(), this);
		Bukkit.getServer().getPluginManager().registerEvents(afk, this);//Register our event with bukkit so it fires
...
//      ^ disabled due to not working yet
		...
		Bukkit.getServer().getWorld("world").getWorldBorder().setSize(20000); //set to our own size we wanted
		afk.startTimer();
	}
